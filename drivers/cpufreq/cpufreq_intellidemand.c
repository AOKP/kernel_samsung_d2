...
int boosted_sampling_rate;
u64 boosted_time_sampling_rate;
unsigned int sampling_rate_current;
				...
				const char *buf, size_t count)
{
	...
	frequency_boosted_time = ktime_to_us(ktime_get());

	if (boosted_sampling_rate) {
		boosted_sampling_rate = 0;
		dbs_tuners_ins.sampling_rate = sampling_rate_current;
	}
	return count;
}
				   ...
				   const char *buf, size_t count)
{
	...
	sampling_rate_current = dbs_tuners_ins.sampling_rate;
	return count;
}
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (dbs_tuners_ins.boosted && policy->cpu == 0) {
		if (ktime_to_us(ktime_get()) - frequency_boosted_time >=
	...
	if (boosted_sampling_rate && policy->cpu == 0) {
		if (ktime_to_us(ktime_get()) - boosted_time_sampling_rate >=
		...
		unsigned int code, int value)
{
	...
	if (enable_dbs_input_event) {
		...
		if (sampling_rate_current > BOOSTED_SAMPLING_RATE) {
			dbs_tuners_ins.sampling_rate = BOOSTED_SAMPLING_RATE;
			boosted_time_sampling_rate = ktime_to_us(ktime_get());
			boosted_sampling_rate = 1;
		}
